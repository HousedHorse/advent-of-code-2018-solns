...
def processInput(events):
    ...
    for time in sortedTimes:
        ...
        # guard wake event
            ...
            guard[gid] += wakeTime - sleepTime # time slept by duration
            # also count each invididual minute slept per sleep cycle
            # (we need this later!!)
            for minute in range(sleepTime,wakeTime+1):
                sleptMinutes[(gid,minute)] += 1

    # find who sleeps the most
    maxTime  = 0
    maxGuard = 0
    for gid in guard:
        if guard[gid] > maxTime:
            maxTime  = guard[gid]
            maxGuard = gid # grab his id

    # find out what minute he sleeps the most
    maxMinuteNum = 0
    maxMinute    = 0
    for (gid,m) in sleptMinutes:
        if gid == maxGuard and sleptMinutes[(gid,m)] > maxMinuteNum:
...
def main():
    ...
    writeSoln(1,processInput(events))
